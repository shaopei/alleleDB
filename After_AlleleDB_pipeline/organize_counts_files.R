...
# combine multiple counts.txt into one SymCals files by "chrm" and "snppos"
# each SymCals will be suffixes like this: SymCals.TF
...
merge_file<-function(file_list, out.name){
 for (file in file_list){
  ...
  head(dataset)
  core.col = names(dataset)[names(dataset) %in% c("chrm", "snppos","ref","phase","mat_all","pat_all", "cnv")]
  SymCls.col = names(dataset)[grep ("^SymCls.",names(dataset))]
  winning.col = names(dataset)[grep ("^SymPval.",names(dataset))]
  print(SymCls.col)
  dataset = dataset [ , c(core.col,SymCls.col, winning.col)]   
  ...
  write.table(dataset_sub, out.name, sep="\t", row.names =F)
  return (dataset_sub)
}

#glob2rx("*_interestingHets.txt")
file_list_c <- list.files(pattern=paste("^.*_",args[2],"\\.txt$",sep = ""))
print (file_list_c)
